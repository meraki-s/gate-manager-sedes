...
import { MsdsValidateDocumentsService } from 'src/app/providers/services/validate-documents/msds-validate-documents.service';
...
export class MsdsComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<MsdsComponent>,
  ...
  ngOnInit(): void {
    this.subscription.add(
      this.authService.user$.subscribe((user) => {
        this.pathStorage = `providers/${this.user?.uid}/msds-pdf/`;
      })
    );

    this.validateDocuments$ = this.msdsValidateDocumentsService
    ...
    try {
      this.subscription.add(
        this.msdsValidateDocumentsService
          .addValidateDocumentsMsds(this.archive.value)
  ...
  async delete(index: number, item?: ValidateDocumentsModel) {
    ...
    try {
      this.subscription.add(
        this.msdsValidateDocumentsService
          .deleteValidateDocumentsMsds(id)
          .subscribe((batch) => {
            if (batch) {
              batch
                .commit()
