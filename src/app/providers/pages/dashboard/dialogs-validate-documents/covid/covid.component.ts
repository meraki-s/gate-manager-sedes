...
import { CovidValidateDocumentsService } from 'src/app/providers/services/validate-documents/covid-validate-documents.service';
...
export class CovidComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.subscription.add(
      this.authService.user$.subscribe((user) => {
        this.pathStorage = `providers/${this.user?.uid}/covid-pdf/`;
      })
    );

    this.validateDocuments$ = this.covidValidateDocumentsService
    ...
    try {
      this.subscription.add(
        this.covidValidateDocumentsService
          .addValidateDocumentsCovid(this.archive.value)
  ...
  async delete(index: number, item?: ValidateDocumentsModel) {
    ...
    try {
      this.subscription.add(
        this.covidValidateDocumentsService
          .deleteValidateDocumentsCovid(id)
          .subscribe((batch) => {
            if (batch) {
              batch
                .commit()
